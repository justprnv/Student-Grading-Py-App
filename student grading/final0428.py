...
class StudentGradingApp:
    ...
    def homepage(self):
        ...
        tk.Button(self.main_frame, text="Grades / Reports", width=30, command=lambda: self.go_to(self.grade_menu)).pack(pady=5)
    # === Student Management ===
    ...
    def add_student(self):
        ...
        if not rocket_id or not re.match(r'^R\d{8}$', rocket_id):
    ...
    def edit_student(self):
        ...
        if not students:
            messagebox.showinfo("None", "No students found.", parent=self.root)
            return
        ids = [s[0] for s in students]
        selected_id = simpledialog.askstring("Edit Student", "Choose Rocket ID:\n" + "\n".join(ids), parent=self.root)
        if selected_id:
            new_name = simpledialog.askstring("Edit Name", "Enter New Name:", parent=self.root)
            if new_name:
                cursor.execute("UPDATE students SET name = ? WHERE rocket_id = ?", (new_name, selected_id))
                conn.commit()
                messagebox.showinfo("Updated", "Student updated.", parent=self.root)
    ...
    def add_class(self):
        class_id = simpledialog.askstring("Class ID", "Enter Class ID:", parent=self.root)
    ...
    def edit_class(self):
        ...
        if not classes:
            messagebox.showinfo("None", "No classes found.", parent=self.root)
            return
        ids = [c[0] for c in classes]
        selected_id = simpledialog.askstring("Edit Class", "Choose Class ID:\n" + "\n".join(ids), parent=self.root)
        if selected_id:
            new_name = simpledialog.askstring("Edit Name", "Enter New Class Name:", parent=self.root)
            if new_name:
                cursor.execute("UPDATE classes SET class_name = ? WHERE class_id = ?", (new_name, selected_id))
                conn.commit()
                messagebox.showinfo("Updated", "Class updated.", parent=self.root)
    ...
    def delete_class(self):
        ...
        if selected_id:
            ...
            if confirm:
                ...
                conn.commit()
                messagebox.showinfo("Deleted", "Class deleted.", parent=self.root)
    ...
    def show_assignment_options(self):
        ...
        if not class_id:
            messagebox.showwarning("Missing", "Select a class first.", parent=self.root)
            return
    ...
    def add_assignment(self, class_id):
        title = simpledialog.askstring("Title", "Assignment Title:", parent=self.root)
        due_date = simpledialog.askstring("Due Date", "Due Date (YYYY-MM-DD):", parent=self.root)
        try:
            max_score = int(simpledialog.askstring("Max Score", "Maximum Score:", parent=self.root))
        except:
            messagebox.showerror("Invalid", "Max score must be a number.", parent=self.root)
            return
        type_ = simpledialog.askstring("Type", "Homework or Test:", parent=self.root)
        if type_:
            ...
            conn.commit()
            messagebox.showinfo("Success", "Assignment added.", parent=self.root)

    def edit_assignment(self, class_id):
        cursor.execute("SELECT id, title FROM assignments WHERE class_id = ?", (class_id,))
        assignments = cursor.fetchall()
        if not assignments:
            messagebox.showinfo("None", "No assignments found.", parent=self.root)
            return
        ids_titles = [f"{aid}: {title}" for aid, title in assignments]
        selected = simpledialog.askstring("Edit Assignment", "Select Assignment:\n" + "\n".join(ids_titles), parent=self.root)
        if selected:
            assignment_id = int(selected.split(":")[0])
            new_title = simpledialog.askstring("New Title", "New Assignment Title:", parent=self.root)
            new_due_date = simpledialog.askstring("New Due Date", "New Due Date (YYYY-MM-DD):", parent=self.root)
            new_type = simpledialog.askstring("New Type", "Homework or Test:", parent=self.root)
            try:
                new_max_score = int(simpledialog.askstring("New Max Score", "New Maximum Score:", parent=self.root))
            except:
                messagebox.showerror("Invalid", "Max score must be a number.", parent=self.root)
                return
            if new_title and new_due_date and new_type:
                ...
                conn.commit()
                messagebox.showinfo("Updated", "Assignment updated.", parent=self.root)

    def delete_assignment(self, class_id):
        cursor.execute("SELECT id, title FROM assignments WHERE class_id = ?", (class_id,))
        assignments = cursor.fetchall()
        if not assignments:
            messagebox.showinfo("None", "No assignments to delete.", parent=self.root)
            return
        ids_titles = [f"{aid}: {title}" for aid, title in assignments]
        selected = simpledialog.askstring("Delete Assignment", "Select Assignment:\n" + "\n".join(ids_titles), parent=self.root)
        if selected:
            assignment_id = int(selected.split(":")[0])
            confirm = messagebox.askyesno("Confirm", "Delete this assignment?", parent=self.root)
            if confirm:
                cursor.execute("DELETE FROM assignments WHERE id = ?", (assignment_id,))
                conn.commit()
                messagebox.showinfo("Deleted", "Assignment deleted.", parent=self.root)
    ...
    # === Grade Management and Student Reports ===
    def grade_menu(self):
        self.clear_frame()
        ...
        classes = [row[0] for row in cursor.fetchall()]
        if not classes:
            messagebox.showinfo("None", "No classes available.", parent=self.root)
            return
    ...
    def grade_class_interface(self):
        ...
        if not students:
            messagebox.showinfo("None", "No students available.", parent=self.root)
            return
        ...
        if not assignments:
            messagebox.showinfo("None", "No assignments available.", parent=self.root)
            return
    ...
    def submit_or_update_grade(self):
        ...
        try:
            ...
            messagebox.showerror("Invalid", "Score must be a number.", parent=self.root)
            return

        cursor.execute("SELECT id FROM assignments WHERE title = ?", (assignment_title,))
        assignment_id_result = cursor.fetchone()
        if not assignment_id_result:
            messagebox.showerror("Invalid", "Assignment not found.", parent=self.root)
            return
        assignment_id = assignment_id_result[0]
        ...
        conn.commit()
        messagebox.showinfo("Success", "Grade saved.", parent=self.root)
    ...
    def view_student_report(self):
        ...
        choice = simpledialog.askstring("Search By", "Search by: Rocket ID or Name?", parent=self.root)
        if not choice:
            return

        if choice.lower().startswith('r'):
            ids = [r for r, _ in students]
            student_id = simpledialog.askstring("Select Student", "Rocket IDs:\n" + "\n".join(ids), parent=self.root)
        else:
            names = [n for _, n in students]
            name = simpledialog.askstring("Select Student", "Student Names:\n" + "\n".join(names), parent=self.root)
            if name:
                cursor.execute("SELECT rocket_id FROM students WHERE name = ?", (name,))
    ...
    def export_csv_dropdown(self):
        ...
        if not class_ids:
            messagebox.showinfo("None", "No classes to export.", parent=self.root)
            return
        class_id = simpledialog.askstring("Export Class", "Select Class ID:\n" + "\n".join(class_ids), parent=self.root)
        if class_id:
            self.export_csv(class_id)

    def export_csv(self, class_id):
        file_path = filedialog.asksaveasfilename(defaultextension=".csv", parent=self.root)
        if file_path:
            cursor.execute('''
            ...
            with open(file_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Rocket ID", "Student Name", "Assignment Title", "Score"])
                for row in cursor.fetchall():
                    writer.writerow(row)
            messagebox.showinfo("Exported", "Class CSV exported.", parent=self.root)

    def export_all_data(self):
        file_path = filedialog.asksaveasfilename(defaultextension=".csv", parent=self.root)
        if file_path:
            cursor.execute('''
                SELECT s.rocket_id, s.name, c.class_id, c.class_name, a.id, a.title, a.type, a.due_date, a.max_score, g.score
                FROM students s
            ...
            with open(file_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Rocket ID", "Name", "Class ID", "Class Name", "Assignment ID", "Assignment Title", "Type", "Due Date", "Max Score", "Score"])
                for row in cursor.fetchall():
                    writer.writerow(row)
            messagebox.showinfo("Exported", "All Data exported.", parent=self.root)
